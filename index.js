...
function addMetadata(packname, author) {	
	if (!packname) packname = 'rosse'; if (!author) author = 'Tukibot';	
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Het @${num.split('@')[0]} bienvenido a *${mdata.subject}* recuerda leer las reglas.`
                          client.sendMessage(mdata.id, teks, MessageType.text, { contextInfo: {"mentionedJid": [num]}})
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				teks = `Uy @${num.split('@')[0]} se fue 👋`
				client.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
  ...
  client.updatePresence(from, Presence.composing) 
	...
	case 'creador':
		...
		client.sendMessage(from, 'Arriba está el número del creador del bot rosse\nAhi puedes resolver tus preguntas y errores :)',MessageType.text, { quoted: mek} )
               ...
               break
				...
				case 'adminlist':
					client.updatePresence(from, Presence.composing) 
                                        if (!isUser) return reply(mess.only.daftarB)
					if (!isGroup) return reply(mess.only.group)
					teks = `Lista De Los Administradores Del Grupo*${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
				...
				case 'nsfwass':
					...
					case 'nsfwsidebobs':
						try {
							...
							buffer = await getBuffer(res.url)
							client.sendMessage(from, buffer, image, {quoted: mek, caption: 'La vieja de toledo, tremenda puta'})
						} catch (e) {
							console.log(`Error :`, color(e,'red'))
							reply('❌ *ERROR* ❌')
						}
	...
	if (budy.startsWith(`ay me cago en la puta`)) {
        const none = fs.readFileSync('./mp3/dross1.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }		
	if (budy.startsWith(`holaa`)) {
        const none = fs.readFileSync('./mp3/dross2.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }	
	if (budy.startsWith(`hola coños de su madre`)) {
        const none = fs.readFileSync('./mp3/dross3.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }	
	if (budy.startsWith(`lol`)) {
        const none = fs.readFileSync('./mp3/dross4.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }	
	if (budy.startsWith(`😂😂😂`)) {
        const none = fs.readFileSync('./mp3/dross5.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }	
	if (budy.startsWith(`AAA`)) {
        const none = fs.readFileSync('./mp3/dross6.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }	
	if (budy.startsWith(`jajaja xdxdxd`)) {
        const none = fs.readFileSync('./mp3/dross7.mp3');
		client.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }	
